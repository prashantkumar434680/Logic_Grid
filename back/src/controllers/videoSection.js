...
const saveVideoMetadata = async (req,res)=>{
    try{
        ...
        if(existingVideo){
            return res.status(400).json({
                error: "Video already exists for this problem and user"
            });
        }
        ...
        // create video solution document
...
const VideoDelete = async (req,res)=>{
    try{
        ...
        if(
            video.userId.toString() !== userId.toString() &&
...
const getVideoByProblemId = async (req, res) => {
    try {
        ...
        if (!video) {
            return res.status(404).json({
                error: "No video found for this problem"
            });
        }
        ...
        res.status(200).json({
            ...
            uploadedBy: video.userId
                ? `${video.userId.firstName} ${video.userId.lastName || ''}`.trim()
                : 'Admin',
